    ...
    <div className="p-6 space-y-5">
      ...
      <div className="flex items-center gap-3 mb-2">
        ...
        <div className="flex items-center gap-3 min-w-0">
          <h1 className="text-xl font-semibold text-[#0f172a] dark:text-gray-100 truncate">
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-2.5 bg-[#f8fafc] dark:bg-gray-700 rounded-lg border border-[#e5e7eb] dark:border-gray-700">
        <div className="flex items-center gap-1.5 text-sm">
          ...
          <span className="text-[#64748b] dark:text-gray-400">优先级</span>
        ...
        <div className="flex items-center gap-1.5 text-sm">
          ...
          <span className="text-[#64748b] dark:text-gray-400">SLA</span>
        ...
        <span className="text-[#e5e7eb]">|</span>
        <div className="flex items-center gap-1.5 text-sm">
          <FileText className="w-3.5 h-3.5 text-[#94a3b8] dark:text-gray-500" />
          <span className="text-[#64748b] dark:text-gray-400">工单号</span>
          <span className="text-[#374151] dark:text-gray-300 font-medium">{workOrder?.workOrderNo ?? '—'}</span>
        </div>
        <div className="ml-auto flex items-center gap-1.5 text-sm">
          <span className="text-[#94a3b8] dark:text-gray-500">申请人</span>
          <span className="text-[#374151] dark:text-gray-300">{workOrder?.reporterName ?? '—'}</span>
          {workOrder?.deptName && (
            <><ChevronRight className="w-3 h-3 text-[#cbd5e1]" /><span className="text-[#64748b] dark:text-gray-400">{workOrder.deptName}</span></>
          )}
        </div>
      </div>
      ...
      <div className="flex gap-1 border-b border-[#e5e7eb] dark:border-gray-700">
        <button
          ...
          className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'detail' ? 'border-[#2563eb] text-[#2563eb] dark:text-blue-400' : 'border-transparent text-[#64748b] dark:text-gray-400 hover:text-[#374151] dark:text-gray-300'
          }`}
        >
        ...
        {showExecutionTab && (
          <button
            ...
            className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
              activeTab === 'execution' ? 'border-[#2563eb] text-[#2563eb] dark:text-blue-400' : 'border-transparent text-[#64748b] dark:text-gray-400 hover:text-[#374151] dark:text-gray-300'
            }`}
          >
            ...
            <CardContent className="space-y-3">
              {[
                ...
                <div key={String(label)} className="flex gap-4 text-sm">
                  <span className="w-20 text-[#94a3b8] dark:text-gray-500 flex-shrink-0">{label}</span>
                  <span className="text-[#374151] dark:text-gray-300">{String(value ?? '—')}</span>
                </div>
              ))}
              {/* 附件展示 */}
                        ...
                        <div key={index} className="group relative">
                          {isImage ? (
                            <a href={url} target="_blank" rel="noopener noreferrer"
                              className="block w-20 h-20 rounded-lg border border-[#e5e7eb] dark:border-gray-700 overflow-hidden hover:border-[#2563eb] transition-colors">
                            ...
                            <a href={url} target="_blank" rel="noopener noreferrer"
                              className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg border border-[#e5e7eb] dark:border-gray-700 hover:border-[#2563eb] transition-colors text-sm">
                              <FileText className="w-4 h-4 text-[#64748b] dark:text-gray-400" />
                              <span className="text-[#374151] dark:text-gray-300 max-w-[120px] truncate">{fileName}</span>
                              <Download className="w-3.5 h-3.5 text-[#94a3b8] dark:text-gray-500" />
                            </a>
                          )}
                        </div>
                      );
      ...
      <Dialog open={approveDialog} onOpenChange={setApproveDialog}>
        <DialogContent>
          ...
          <div className="px-6 py-4">
            <p className="text-sm text-[#374151] dark:text-gray-300">确定要通过工单「{workOrder?.title ?? ''}」的审批吗？此操作不可撤销。</p>
          </div>
          <DialogFooter>
      ...
      <Dialog open={rejectDialog} onOpenChange={setRejectDialog}>
        <DialogContent>
          ...
          <div className="px-6 py-4">
            <label className="block text-sm font-medium text-[#374151] dark:text-gray-300 mb-2">驳回原因 <span className="text-red-500">*</span></label>
            <textarea rows={4} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)}
              placeholder="请填写驳回原因（必填，最多 500 字）..."
              className="w-full px-3 py-2 text-sm border border-[#e5e7eb] dark:border-gray-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-[#3b82f6] resize-none" maxLength={500} />
            <p className="text-xs text-[#94a3b8] dark:text-gray-500 mt-1 text-right">{rejectReason.length}/500</p>
          </div>
          <DialogFooter>
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <textarea value={holdReason} onChange={e => setHoldReason(e.target.value)}
                className="w-full h-24 rounded-xl border border-[#d7deea] dark:border-gray-600 text-sm p-3 resize-none focus:outline-none focus:ring-2 focus:ring-[#2563eb]/20"
                placeholder="请说明挂起原因..." />
            </div>
            <div>
              ...
              <input type="datetime-local" value={holdResumeAt} onChange={e => setHoldResumeAt(e.target.value)}
                className="w-full h-11 rounded-xl border border-[#d7deea] dark:border-gray-600 text-sm px-3" />
            </div>
          </div>
          <DialogFooter>
      ...
      <Card className="shadow-sm border-[#e5e7eb] dark:border-gray-700">
        <CardContent className="p-6">
          <CommentSection businessType="WORK_ORDER" businessId={orderId} />
        </CardContent>
      </Card>
    </div>
  );
