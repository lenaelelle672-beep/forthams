...
import { Plus, Pencil, Trash2, RefreshCw, Calendar, Play, ChevronLeft, ChevronRight, AlertTriangle, AlertCircle, Clock, LayoutGrid, Table as TableIcon, Wrench, Search, ExternalLink, ClipboardList } from 'lucide-react';
...
function PlanFormDialog({ open, plan, submitting, assetOptions, onClose, onSubmit }: {
  open: boolean;
  plan: MaintenancePlan | null;
  submitting: boolean;
  assetOptions?: { records?: AssetListItem[] };
  onClose: () => void;
        ...
        <form ref={formRef} onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-slate-700">关联资产 *</label>
              <Select value={form.assetId ? String(form.assetId) : '0'} onValueChange={v => set('assetId', Number(v))}>
                <SelectItem value="0">请选择资产</SelectItem>
                {(assetOptions?.records ?? []).map(a => (
                  <SelectItem key={a.id} value={String(a.id)}>{a.assetName} ({a.assetNo})</SelectItem>
                ))}
              </Select>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            <div className="flex flex-col gap-1.5">
              <label className="text-sm font-medium text-slate-700">触发类型</label>
            ...
            {form.triggerType === 'weekly' && (
              <div className="flex flex-col gap-1.5">
                <label className="text-sm font-medium text-slate-700">星期几</label>
                <Select value={String(form.dayOfWeek ?? 1)} onValueChange={v => set('dayOfWeek', Number(v))}>
                  <SelectItem value="1">周一</SelectItem><SelectItem value="2">周二</SelectItem><SelectItem value="3">周三</SelectItem>
                  <SelectItem value="4">周四</SelectItem><SelectItem value="5">周五</SelectItem><SelectItem value="6">周六</SelectItem><SelectItem value="7">周日</SelectItem>
                </Select>
              </div>
            )}
          ...
          {/* 默认维保内容 */}
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium text-slate-700">默认维保内容</label>
            <textarea value={form.defaultContent || ''} onChange={e => set('defaultContent', e.target.value)}
              rows={3} placeholder="如：检查设备运行状态、清洁滤网、更新固件..."
              className="px-3 py-2 rounded-lg border border-slate-200 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-300 resize-none transition-colors" />
          </div>
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="px-6 py-4">
          <p className="text-sm text-slate-500">
            确定要删除维保计划「<span className="font-medium text-slate-900">{plan?.planName}</span>」吗？
          </p>
        </div>
        <DialogFooter>
...
function CalendarView({ plans, assetMap, navigate }: { plans: MaintenancePlan[]; assetMap: Map<number, AssetListItem>; navigate: ReturnType<typeof useNavigate> }) {
    ...
    <div className="px-5 py-4">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-base font-semibold text-slate-900 min-w-[140px] text-center select-none">
            {monthLabel}
          </h3>
      ...
      <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-slate-200">
          {weekDayLabels.map(label => (
            <div key={label}
              className={`px-2 py-2.5 text-center text-xs font-semibold text-slate-500 bg-slate-50
            ...
            return (
              ...
              >
                ...
                <div className={`
                  ...
                  ${dayData.isToday ? 'bg-blue-600 text-white' : isWeekend ? 'text-red-400' : 'text-slate-700'}
                `}>
                  {dayData.day}
                </div>
                {/* 计划标记 */}
                ...
                return (
                  <div key={p.id} className="flex items-center justify-between p-2.5 bg-slate-50 rounded-lg border border-slate-100">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-slate-900 truncate">{p.planName}</p>
                      <p className="text-xs text-slate-500 mt-0.5">
                        {asset ? (
                          <button onClick={() => navigate(`/assets/${p.assetId}`)} className="text-indigo-600 hover:text-indigo-700 hover:underline font-medium">
                            {asset.assetName}
                          </button>
                        ) : `资产 #${p.assetId}`}
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <button onClick={() => navigate('/maintenance')}
                            className="p-1.5 rounded-lg border border-slate-200 bg-white text-slate-500 hover:bg-slate-50 hover:text-blue-600 transition-colors">
                            <ClipboardList className="w-3.5 h-3.5" />
                          </button>
                        </TooltipTrigger>
                        <TooltipContent>查看维保记录</TooltipContent>
                      </Tooltip>
                    </div>
                  </div>
                );
...
export default function MaintenancePlanPage() {
  ...
  const columns: Column<MaintenancePlan>[] = [
          ...
          <div className="flex items-center gap-1.5">
            ...
            <div>
              <Tooltip>
                <TooltipTrigger asChild>
                  <button onClick={() => navigate('/maintenance')}
                    className="font-medium text-slate-900 leading-tight hover:text-blue-600 hover:underline cursor-pointer text-left">
                    {row.planName}
                  </button>
                </TooltipTrigger>
                <TooltipContent>点击查看维保记录</TooltipContent>
              </Tooltip>
    ...
    {
      key: 'assetId', title: '关联资产', width: 160,
      render: (v) => {
        const asset = assetMap.get(Number(v));
        return asset ? (
          <Tooltip>
            <TooltipTrigger asChild>
              <button onClick={() => navigate(`/assets/${v}`)}
                className="inline-flex items-center gap-1.5 rounded-md bg-indigo-50 px-2 py-0.5 text-xs font-medium text-indigo-700 ring-1 ring-inset ring-indigo-100 hover:bg-indigo-100 transition-colors max-w-[140px]">
      ...
      render: (_v, row) => {
        ...
        return (
          <div className="flex flex-col gap-0.5">
            <span className={`text-xs ${risk === 'overdue' ? 'text-red-700 font-semibold' : risk === 'urgent' ? 'text-orange-700 font-medium' : 'text-slate-700'}`}>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <button onClick={(e) => { e.stopPropagation(); setEditingPlan(row); setDialogOpen(true); }}
                className="inline-flex h-7 items-center gap-1 rounded-lg border border-slate-200 bg-white px-2 text-xs font-medium text-slate-500 hover:bg-slate-50 hover:text-blue-600 transition-colors">
            ...
            <Tooltip>
              <TooltipTrigger asChild>
                <button onClick={(e) => { e.stopPropagation(); generateMut.mutate(row.id); }}
                  ...
                  className="inline-flex h-7 items-center gap-1 rounded-lg border border-emerald-200 bg-white px-2 text-xs font-medium text-emerald-600 hover:bg-emerald-50 transition-colors disabled:opacity-40">
                  <Play className="w-3.5 h-3.5" />
                </button>
              </TooltipTrigger>
              <TooltipContent>手动生成维保记录</TooltipContent>
            </Tooltip>
          )}
          <Tooltip>
            <TooltipTrigger asChild>
              <button onClick={(e) => { e.stopPropagation(); setDeletingPlan(row); setDeleteDialogOpen(true); }}
                className="inline-flex h-7 items-center gap-1 rounded-lg border border-red-200 bg-white px-2 text-xs font-medium text-red-500 hover:bg-red-50 transition-colors">
      ...
      <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-6">
            ...
            <div className="flex items-center gap-2">
              <div className="flex bg-slate-100 rounded-lg p-0.5">
                <button onClick={() => setViewMode('table')}
                  className={`p-1.5 rounded-md transition-colors ${viewMode === 'table' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`} title="表格视图">
                  <TableIcon className="w-4 h-4" />
                </button>
                <button onClick={() => setViewMode('card')}
                  className={`p-1.5 rounded-md transition-colors ${viewMode === 'card' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`} title="卡片视图">
                  <LayoutGrid className="w-4 h-4" />
                </button>
                <button onClick={() => setViewMode('calendar')}
                  className={`p-1.5 rounded-md transition-colors ${viewMode === 'calendar' ? 'bg-white shadow-sm text-slate-900' : 'text-slate-500 hover:text-slate-700'}`} title="日历视图">
            ...
            <div className="grid grid-cols-4 divide-x divide-slate-100 border-t border-slate-100">
              <div className="flex items-center gap-3 px-6 py-4">
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-500">计划总数</p>
                </div>
              </div>
              <div className="flex items-center gap-3 px-6 py-4">
                ...
                <div>
                  <p className={`text-2xl font-bold tabular-nums ${riskStats.overdue > 0 ? 'text-red-700' : 'text-slate-400'}`}>{riskStats.overdue}</p>
              ...
              <div className="flex items-center gap-3 px-6 py-4">
                ...
                <div>
                  <p className={`text-2xl font-bold tabular-nums ${riskStats.urgent > 0 ? 'text-orange-700' : 'text-slate-400'}`}>{riskStats.urgent}</p>
              ...
              <div className="flex items-center gap-3 px-6 py-4">
                ...
                <div>
                  <p className={`text-2xl font-bold tabular-nums ${riskStats.warning > 0 ? 'text-yellow-700' : 'text-slate-400'}`}>{riskStats.warning}</p>
        ...
        {viewMode === 'calendar' ? (
          <Card className="overflow-hidden rounded-2xl border-slate-200/80 shadow-sm">
            <CalendarView plans={upcomingData || []} assetMap={assetMap} navigate={navigate} />
          </Card>
        ) : (
          <Card className="overflow-hidden rounded-2xl border-slate-200/80 shadow-sm">
            {/* Toolbar */}
            <div className="flex items-center gap-3 border-b border-slate-100 px-5 py-3.5 flex-wrap">
              ...
              <button
                ...
                className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors shadow-sm"
              >
              ...
              <button
                ...
                disabled={isFetching}
                className="inline-flex h-9 items-center gap-1.5 rounded-lg border border-slate-200 bg-white px-3.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors disabled:opacity-40 shadow-sm"
              >
                      ...
                      return (
                        <div key={p.id}
                          ...
                          <div className="flex items-start justify-between gap-2 mb-3">
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1.5">
                                ...
                                <button onClick={() => navigate('/maintenance')}
                                  className="text-sm font-semibold text-slate-900 truncate hover:text-blue-600 hover:underline text-left">
                                  {p.planName}
                                </button>
                              </div>
                              <p className="text-xs text-slate-400 mt-0.5">
                                {cardAsset ? (
                                  <button onClick={() => navigate(`/assets/${p.assetId}`)}
                                    className="inline-flex items-center gap-1 rounded bg-indigo-50 px-1.5 py-px text-[11px] font-medium text-indigo-600 ring-1 ring-inset ring-indigo-100 hover:bg-indigo-100 transition-colors max-w-[160px]">
                                    <span className="truncate">{cardAsset.assetName}</span>
                                    <ExternalLink className="w-2.5 h-2.5 shrink-0 opacity-60" />
                                  </button>
                                ) : (
                                  <button onClick={() => navigate(`/assets/${p.assetId}`)}
                                    className="inline-flex items-center rounded bg-indigo-50 px-1.5 py-px text-[11px] font-medium text-indigo-600 ring-1 ring-inset ring-indigo-100 hover:bg-indigo-100 transition-colors">
                                    #{p.assetId}
                                  </button>
                                )}
                              </p>
                          ...
                          <div className="space-y-2 mb-3 text-xs">
                            ...
                            <div className="flex items-center gap-2">
                              {p.defaultExecutor ? (
                                <>
                                  ...
                                  <span className="text-slate-700">{p.defaultExecutor}</span>
                                </>
                              ) : (
                            ...
                            <div className="flex items-center gap-2">
                              <span className={`text-[10px] ${risk === 'overdue' ? 'text-red-700 font-semibold' : risk === 'urgent' ? 'text-orange-700 font-medium' : 'text-slate-700'}`}>
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-slate-100">
                            ...
                            <div className="flex items-center gap-0.5">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <button onClick={() => navigate('/maintenance')}
                                    className="p-1 rounded text-slate-500 hover:bg-slate-100 hover:text-blue-600 transition-colors">
                                    <ClipboardList className="w-3 h-3" />
                                  </button>
                                </TooltipTrigger>
                                <TooltipContent>维保记录</TooltipContent>
                              </Tooltip>
                              <Tooltip>
                                <TooltipTrigger asChild>
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <button onClick={() => generateMut.mutate(p.id)} disabled={generateMut.isPending}
                                      className="p-1 rounded text-slate-500 hover:bg-slate-100 hover:text-emerald-600 transition-colors disabled:opacity-40">
        ...
        <PlanFormDialog
          ...
          assetOptions={assetOptions as any}
