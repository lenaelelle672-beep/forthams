...
export default function SystemNumberingRulesWorkbenchPage({
  ...
  return (
    <section className="space-y-5" data-embedded={embeddedInWorkbench} data-system-numbering-rules="workbench-v3">
      ...
      {loading ? <div role="status" aria-live="polite" className="text-sm text-slate-500">编号规则只读目录加载中...</div> : null}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
        <section className="rounded-2xl border border-slate-200 p-4" aria-label="编号规则只读列表">
          ...
          {rules.length === 0 && !loading ? <h3 className="rounded-2xl border border-dashed border-slate-200 p-6 text-sm font-medium text-slate-500">暂无编号规则；空态不代表序列分配、并发唯一或创建链路闭环。</h3> : null}
