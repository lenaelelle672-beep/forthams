...
export default function SystemAssetCategoryWorkbenchPage({
  ...
  return (
    <section className="space-y-5" data-embedded={embeddedInWorkbench}>
      ...
      {loading ? <div className="text-sm text-slate-500">资产分类加载中...</div> : null}
      {!loading && !error && !hasRecords && !hasTree ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-sm text-slate-500">暂无资产分类数据。</div>
      ) : null}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(280px,0.75fr)]">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
          ...
          {hasTree ? renderTree(tree) : <p className="text-sm text-slate-500">暂无分类树节点。</p>}
        </div>
      </div>
    </section>
  );
