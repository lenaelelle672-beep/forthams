...
export default function SystemDocCenterWorkbenchPage({
  ...
  return (
    <section className="space-y-5" data-embedded={embeddedInWorkbench}>
      ...
      {loading ? <div className="text-sm text-slate-500">文档加载中...</div> : null}
      {!loading && !error && visibleRecords.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-sm text-slate-500">暂无文档记录。</div>
      ) : null}
            ...
            <tbody className="divide-y divide-slate-100">
              {visibleRecords.map((record) => (
                <tr key={record.id}>
                  ...
                  <td className="py-2 pr-3 text-xs text-slate-400">{record.updatedAt ?? record.publishedAt ?? '-'}</td>
                </tr>
              ))}
            </tbody>
          </table>
