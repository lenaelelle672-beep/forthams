...
export default function SystemDataPermissionsWorkbenchPage({
  ...
  return (
    <section className="space-y-5" data-embedded={embeddedInWorkbench}>
      ...
      {loading ? <div className="text-sm text-slate-500">数据权限加载中...</div> : null}
      {!loading && !error && visibleRoles.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-slate-200 p-6 text-sm text-slate-500">暂无符合条件的角色数据范围。</div>
      ) : null}
          ...
          <table className="min-w-full text-left text-sm">
            <thead className="text-xs text-slate-500">
              <tr>
                ...
                <th className="py-2 pr-3">数据范围</th>
                <th className="py-2 pr-3">风险说明</th>
              </tr>
            </thead>
