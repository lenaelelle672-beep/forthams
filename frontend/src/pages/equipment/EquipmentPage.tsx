...
export default function EquipmentPage() {
  ...
  const { isLoading: detailLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const resp = await getAssetById(Number(assetId));
  ...
  const filtered = equipment.filter((e) => {
    ...
    const matchMaintenanceStatus = maintenanceStatusFilter === 'all' || e.maintenanceStatus === maintenanceStatusFilter;
    return matchSearch && matchStatus && matchMaintenanceStatus;
  });
  ...
  const maintenanceStatusFilters = [
    { key: 'all', label: '全部' },
    { key: '正常', label: '正常' },
    { key: '即将到期', label: '即将到期' },
    { key: '已过期', label: '已过期' },
      ...
      <div className="mx-auto flex w-full max-w-[1480px] flex-col gap-6">
        ...
        <Card className="overflow-hidden rounded-2xl border-slate-200/80 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-2 border-b border-slate-100 px-5 py-3">
            ...
            {maintenanceStatusFilters.map(({ key, label }) => (
              <button
                key={key}
                onClick={() => setMaintenanceStatusFilter(key)}
                className={`rounded-full border px-3 py-1 text-xs font-medium transition-colors ${
                  maintenanceStatusFilter === key
                    ? 'border-blue-200 bg-blue-50 text-blue-700 shadow-sm'
                    : 'border-transparent bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
        ...
        <Dialog open={!!detailItem} onOpenChange={(open) => { if (!open) { setDetailItem(null); setDetailAsset(null); } }}>
          <DialogContent className="max-w-lg">
            ...
            <DialogFooter>
              <Button variant="secondary" onClick={() => { setDetailItem(null); setDetailAsset(null); }}>
                关闭
              </Button>
              <Button onClick={() => { if (detailItem) navigate('/assets/' + detailItem.id); }}>
                前往资产详情
              </Button>
            </DialogFooter>
          </DialogContent>
