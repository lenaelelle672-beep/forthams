...
describe('useGisAssets', () => {
  ...
  it('should render an empty map dataset when gis api is forbidden', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    mockedGisService.getAssets.mockRejectedValueOnce(Object.assign(new Error('Forbidden'), { status: 403 }));

    const { result } = renderHook(() => useGisAssets(), { wrapper: createWrapper() });

    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(result.current.data).toEqual([]);
    expect(warnSpy).toHaveBeenCalledWith('[GIS] 资产定位接口不可用或权限不足，展示空地图数据');
    warnSpy.mockRestore();
  });
});
