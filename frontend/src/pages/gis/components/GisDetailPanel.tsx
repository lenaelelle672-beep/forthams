...
export interface GisDetailPanelProps {
  ...
  /** 是否显示跳转到资产详情按钮 */
  onViewAsset?: (asset: GisAsset) => void;
}
...
export const GisDetailPanel: React.FC<GisDetailPanelProps> = ({
  ...
  return (
    ...
    >
      ...
      {(onViewAsset || onViewEnergy) && (
        <div className="px-4 py-2.5 border-t border-gray-100 space-y-1">
          {onViewAsset && (
            <button
              type="button"
              onClick={() => onViewAsset(asset)}
              className="w-full flex items-center justify-center gap-1.5 text-xs text-indigo-600 hover:bg-indigo-50 hover:text-indigo-700 rounded-md py-1.5 transition-colors font-medium"
            >
          ...
          {onViewEnergy && (
            <button
              ...
              className="w-full flex items-center justify-center gap-1.5 text-xs text-blue-600 hover:bg-blue-50 hover:text-blue-700 rounded-md py-1.5 transition-colors"
            >
