...
export default function BorrowListPage() {
  ...
  /* ── Debounced search: sync keywordInput → keyword after 300ms ── */
  useEffect(() => {
  ...
  const handleExportCSV = () => {
    const statusLabel = (s: string) => BORROW_STATUS_CONFIG[s as BorrowStatus]?.label ?? s;
    const headers = ['资产编号', '资产名称', '状态', '借用日期', '预计归还', '用途', '备注'];
    ...
    a.href = url;
    a.download = `borrow-export-${Date.now()}.csv`;
    a.click();
    URL.revokeObjectURL(url);
  };

  /* ── Status counts from visible records ── */
  ...
  const statCards: StatCardDef[] = useMemo(() => [
    ...
    {
      label: '已借出',
      value: statusCounts[BorrowStatus.BORROWED] ?? 0,
      unit: '条',
      icon: FileCheck,
    ...
    {
      label: '已逾期',
      value: statusCounts[BorrowStatus.OVERDUE] ?? 0,
      unit: '条',
      icon: AlertCircle,
    ...
    {
      label: '待审批',
      value: statusCounts[BorrowStatus.PENDING_APPROVAL] ?? 0,
      unit: '条',
      icon: Clock,
          ...
          <div className="border-b border-slate-100 bg-gradient-to-r from-white via-[#fbfdff] to-[#f8fbff] px-5 py-4">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" size="sm" onClick={handleExportCSV} title="导出CSV">
