...
const EnergyDashboardPage: React.FC = () => {
  ...
  const anomalies = useEnergyAnomalies(
    effectiveData?.trend
      ? (Object.fromEntries(
          Object.entries(effectiveData.trend).map(([k, v]) => [k, Number(v)] as const),
  ...
  const suggestions = useMemo(
    () => (effectiveData ? SUGGESTION_TEMPLATES.filter((t) => t.condition(effectiveData)) : []),
    [effectiveData],
  );
  ...
  const gasConsumption = Number(effectiveData?.byType?.GAS || 0);
  ...
  const trendOption = useMemo(() => ({
    ...
    xAxis: {
      ...
      data: effectiveData ? Object.keys(effectiveData.trend || {}) : [],
    ...
    series: [
      {
        type: 'line',
        data: effectiveData ? Object.values(effectiveData.trend || {}) : [],
        smooth: true,
        symbol: 'circle',
        symbolSize: 6,
    ...
    series: [
      {
        ...
        data: effectiveData
          ? Object.entries(effectiveData.byType || {}).map(([key, value]) => ({
  ...
  if (!effectiveData && !isLoading && !isError) {
    return (
      <PageTransition>
