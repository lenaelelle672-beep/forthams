  ...
  const columns: Column<IdleAssetRecord>[] = [
    {
      ...
      width: 120,
      render: (_v, row) => (
        <a className="font-medium text-blue-600 hover:text-blue-700 hover:underline cursor-pointer"
           onClick={() => navigate(`/assets/${row.assetId ?? row.id}`)}>
    ...
    },
    {
      ...
      render: (_v, row) => (
        <div>
          <a className="font-medium text-gray-900 hover:text-blue-600 hover:underline cursor-pointer"
             onClick={() => navigate(`/assets/${row.assetId ?? row.id}`)}>
