...
test.describe('Workbench 正式入口浏览器回归', () => {
  ...
  test('基础维护左侧菜单渲染真实页面级组件而非通用产品壳', async ({ page }) => {
    ...
    await expect(page.getByText('分类树 · 数据字典 · 集成源')).toBeVisible();
    const settingsTreeList = page.locator('.workspace-settings-tree-list');
    ...
    await expect(page.getByLabel('基础维护分类树')).toContainText('编号规则');
    await expect(page.getByLabel('基础维护二级目录')).toContainText('生产设备');
    await expect(page.getByLabel('基础维护状态反馈')).toContainText('暂无待维护配置');
    await expect(page.getByLabel('基础维护状态反馈')).toContainText('供应商资质受限');
