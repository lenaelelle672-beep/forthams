/**
 * @module frontend/src/app/pages/workorders/WorkOrderListPage
 * @description Work Order List Page — real backend API driven with pagination,
 *              search, and status filter tabs.
 *
 * Consumes useWorkOrderList and useWorkOrderMutation hooks from useWorkOrders.ts
 * for all data fetching and mutations. No mock/stub data.
 *
 * Features:
 ...
 * - Navigate to detail page via "查看详情" button
 *
 ...
 *
 * Uses useWorkOrderList hook for data fetching and useWorkOrderMutation
 * for lifecycle operations. All data comes from real backend API.
 *
 * Clicking the "查看详情" button navigates to /workorders/:id which renders
 * the WorkOrderDetailPage with full details and approval actions.
 */
export function WorkOrderListPage() {
  ...
  const { submit, operate, remove } = useWorkOrderMutation();
  ...
  /**
   * Re-fetch with current tab + keyword when they change.
   * Overrides the params passed to the hook.
   */
  useEffect(() => {
    refetch({
      status: activeTab !== "ALL" ? activeTab : undefined,
  ...
  }, [activeTab, debouncedKeyword, page, refetch]);

  // -- lifecycle action handlers -------------------------------------------

  /**
   * Submit a draft work order for approval.
   * Calls POST /api/workorders/{id}/submit via useWorkOrderMutation.
   */
  ...
  const handleComplete = useCallback(
    async (id: number) => {
      try {
        setOperating(id);
        setActionError(null);
        await operate(id, "complete", "处理完成");
        setNotice("工单已完成");
        await refetch();
      } catch (err: unknown) {
        const message = err instanceof Error ? err.message : "操作失败";
  ...
  /**
   * Delete a work order.
   * Calls DELETE /api/workorders/{id} via useWorkOrderMutation.
   */
  ...
  // -- render --------------------------------------------------------------
  return (
    <div className="space-y-6" data-testid="work-order-list-page">
      {/* Page Header */}
                  ...
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm">
                      <div className="flex items-center gap-1">
                        {/* View detail — navigates to WorkOrderDetailPage */}
                        <button
                          onClick={() =>
                          ...
                          title="查看详情"
                          data-testid={`btn-view-detail-${order.id}`}
                        >
