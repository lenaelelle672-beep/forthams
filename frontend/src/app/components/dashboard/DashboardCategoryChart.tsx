/**
 * DashboardCategoryChart — 资产分类分布饼图组件
 *
 * 通过调用 dashboardService.getStats() 获取资产统计数据中的
 * categoryDistribution 字段，使用 Recharts PieChart 渲染分类分布。
 *
 * @module components/dashboard/DashboardCategoryChart
 * @see frontend/src/app/services/dashboardService.ts — DashboardStats.categoryDistribution
 */
 ...
 *
 * 展示资产在各分类下的数量分布，采用 Recharts PieChart（环形）渲染。
 * 当外部未传入 data 时，组件自行从 dashboardService.getStats() 获取数据。
 *
 * @example
 * ```tsx
 * <DashboardCategoryChart />
 * <DashboardCategoryChart data={[{ name: 'IT', value: 50 }]} />
 * ```
 */
