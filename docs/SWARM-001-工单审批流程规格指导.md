...
根据 AC Criteria，Iteration 1 需完成以下交付物：

| 序号 | 文件路径 | 描述 | 优先级 |
|------|----------|------|--------|
| 1 | `frontend/src/types/workorder.types.ts` | 前端工单类型定义 | P0 |
| 2 | `frontend/router/approval.ts` | 审批流程路由配置 | P0 |
| 3 | `backend/state_machine/workorder_state.py` | 工单状态机实现 | P0 |
| 4 | `backend/services/workorder_service.py` | 工单业务服务层 | P0 |
| 5 | `frontend/tests/unit/test_approval_chain.py` | 审批链单元测试 | P1 |
| 6 | `backend/src/main/java/com/ams/service/NotificationService.java` | 通知服务（核心上下文） | P0 |
...
| AC ID | 验收方法 | 状态 | 检查项 |
|-------|---------|------|--------|
| AC-001 | unit_test | pending | 工单提交后状态为 PENDING |
| AC-002 | unit_test | pending | 状态机转换符合状态图定义 |
| AC-003 | static_analysis | pending | AST 静态检查通过，无语法错误 |
