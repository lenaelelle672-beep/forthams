...
| 功能点 | 描述 |
|--------|------|
| 报废申请发起 | 用户可对目标资产提交报废申请，填写报废原因及附件 |
| 状态自动校验 | 系统在校验资产当前状态是否符合报废前置条件 |
| 处置记录生成 | 审批通过后自动生成资产处置流水记录 |
| 审批链路触发 | 根据资产价值/类别匹配对应审批流程 |
| 生命周期状态更新 | 报废完成后更新资产状态为"已报废"，标记退役 |
...
src/
├── models/
│   └── retire_application.py    # 报废申请单数据模型
├── schemas/
│   └── retire_application.py     # Pydantic 请求/响应 schema
└── repositories/
    └── retire_application.py     # 报废申请单仓储层
```

**交付物**:
- `RetireApplication` 模型类
- `CreateRetireApplicationSchema` / `RetireApplicationResponseSchema`
- `RetireApplicationRepository` 基础 CRUD
...
src/
└── services/
    └── retire_service.py         # 报废业务服务层
```

**交付物**:
- `RetireService.create_retire_application()` - 创建报废申请
- `RetireService.validate_asset_retire_eligibility()` - 资产报废资格校验
- 状态校验规则实现
- 权限校验逻辑集成
