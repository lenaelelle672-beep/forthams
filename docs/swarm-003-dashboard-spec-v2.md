...
describe('DashboardStore', () => {
  ...
  describe('fetchMaintenanceAlerts', () => {
    it('应过滤仅返回未来90天内到期项', async () => {
      ...
      const mockData = [
        { assetId: 'A1', dueDate: new Date(today.getTime() + 5 * 86400000).toISOString() }, // 5天后
        { assetId: 'A2', dueDate: new Date(today.getTime() + 100 * 86400000).toISOString() }  // 100天后
      ]
      httpClient.get.mockResolvedValue({ data: mockData })
...
test.describe('仪表板数据看板', () => {
  ...
  test('ATB-2.1: 资产总览统计卡片应正确展示', async ({ page }) => {
    ...
    await expect(totalCard.locator('.stat-value')).not.toHaveText('—') // 非加载态
  })
  ...
  test('ATB-2.3: 维保预警列表应按紧急程度排序', async ({ page }) => {
    ...
    expect(count).toBeLessThanOrEqual(5) // 最多5条
