...
| 文件路径 | 修改内容 | 优先级 |
|----------|----------|--------|
| `frontend/src/router/approval.ts` | 审批流程路由配置 | P0 |
| `frontend/src/app/routes.ts` | 路由注册与布局 | P0 |
| `frontend/src/app/pages/AuditDashboard/components/FilterBar/FilterBar.module.css` | 审批筛选组件样式 | P1 |
| `src/state_machine/approval_state_machine.py` | 状态机引擎与Guard实现 | P0 |
| `src/models/workorder.py` | 工单数据模型扩展 | P0 |
| `src/domain/entities/work_order.py` | 工单领域实体方法 | P0 |
...
### `frontend/src/app/routes.ts` 路由注册
...
| Guard函数 | 校验逻辑 | 返回类型 |
|-----------|----------|----------|
| `evaluate_pending_to_approving` | 校验是否为创建者本人提交 | `ValidationResult` |
| `evaluate_approving_to_approved` | 校验审批人权限 | `ValidationResult` |
| `evaluate_approving_to_rejected` | 校验驳回理由非空 | `ValidationResult` |
| `evaluate_rejected_to_draft` | 校验是否为创建者本人 | `ValidationResult` |
| `evaluate_to_closed` | 校验是否已完成所有节点 | `ValidationResult` |
| `evaluate_optimistic_lock` | 校验version字段一致性 | `ValidationResult` |
