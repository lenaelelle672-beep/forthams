...
| 约束编号 | 约束类型 | 约束描述 |
|----------|----------|----------|
| C-01 | 禁止直接操作 DOM | 所有状态更新必须通过 React 响应式系统，禁止使用 `ref.current` 或原生 DOM 操作 |
| C-02 | 禁止绕过 Service 层 | UI 组件禁止直接调用 Axios 实例，必须经由 ApprovalService 封装方法 |
| C-03 | 禁止同步 XHR | 所有 HTTP 请求必须为异步，禁止使用 `async: false` 配置 |
| C-04 | 状态不可变原则 | Store 中的 approval 状态禁止直接修改，必须通过 actions 更新 |
...
| Test ID | 测试描述 | 测试输入 | 预期输出 | 物理验证方式 |
|---------|----------|----------|----------|--------------|
| UT-09 | Service 状态变更触发 UI 更新 | 外部修改 `store.approvalList` | React 组件重新渲染 | Vitest + `expect(screen.getByText(...)).toBeInTheDocument()` |
| UT-10 | UI 操作触发 Service 更新 | 用户触发 `handleApprove()` | `service.updateApprovalStatus()` 被调用 | Vitest + `vi.fn()` spy + `fireEvent.click()` |
| UT-11 | 并发操作互斥验证 | 同时触发多个审批操作 | 第二个操作进入 `pendingOperations` 队列 | Vitest + `expect(store.pendingOperations.size).toBe(1)` |

### 4.2 组件测试验收

**测试文件**: `tests/unit/components/approvalList.test.tsx`

| Test ID | 测试描述 | 测试输入 | 预期输出 | 物理验证方式 |
|---------|----------|----------|----------|--------------|
| UT-12 | 列表渲染正确数量 | `approvalList = [A, B, C]` | 渲染 3 个 `ApprovalItem` 组件 | Testing Library + `screen.getAllByRole('listitem').length` |
| UT-13 | 加载状态显示 | `loadingState.isLoading = true` | 显示 Loading 骨架屏 | Testing Library + `expect(screen.getByTestId('loading-skeleton')).toBeVisible()` |
| UT-14 | 空状态显示 | `approvalList = []` 且 `!loading` | 显示空状态插画与文案 | Testing Library + `expect(screen.getByTestId('empty-state')).toBeVisible()` |
| UT-15 | 错误状态显示 | `errorState` 非空 | 显示错误提示与重试按钮 | Testing Library + `expect(screen.getByTestId('error-alert')).toBeVisible()` |
...
Layer 1: 基础设施层
    ↓ 依赖: axios 实例, React Router, Zustand
...
| 1.3 | 配置 React Router 审批路由 | `src/router/approval.tsx` | 无 |
| 1.4 | 初始化 Zustand Store 骨架 | `src/stores/approvalStore.ts` | 无 |

#### Phase 2: ApprovalService 核心实现 (预计 2d)

| 序号 | 任务 | 交付物 | 依赖前置 |
|------|------|--------|----------|
| 2.1 | 定义 TypeScript 类型接口 | `src/types/audit.types.ts` | 无 |
...
#### Phase 3: Zustand Store 双向绑定实现 (预计 1.5d)
...
| 3.4 | 实现 State 更新逻辑 | `src/stores/approvalStore.ts` | 3.3 |
...
| 序号 | 任务 | 交付物 | 依赖前置 |
|------|------|--------|----------|
| 4.1 | 开发 `ApprovalListItem` 组件 | `src/components/approval/ApprovalListItem.tsx` | 3.5 |
| 4.2 | 开发 `ApprovalStatusBadge` 组件 | `src/components/approval/ApprovalStatusBadge.tsx` | 4.1 |
| 4.3 | 开发 `ApprovalFilter` 组件 | `src/components/approval/ApprovalFilter.tsx` | 4.2 |
| 4.4 | 开发 `ApprovalActionPanel` 组件 | `src/components/approval/ApprovalActionPanel.tsx` | 3.5 |
| 4.5 | 组件单元测试覆盖 | `tests/unit/components/*.test.tsx` | 4.1-4.4 |

#### Phase 5: 视图层组装 (预计 1d)

| 序号 | 任务 | 交付物 | 依赖前置 |
|------|------|--------|----------|
| 5.1 | 组装 `ApprovalListView` | `src/pages/approval/ApprovalListView.tsx` | 4.1-4.3 |
| 5.2 | 组装 `ApprovalDetailView` | `src/pages/approval/ApprovalDetailView.tsx` | 4.4 |
| 5.3 | 集成路由守卫（权限控制） | `src/router/approval.tsx` | 5.1, 5.2 |
| 5.4 | 视图层集成测试 | `tests/unit/views/*.test.tsx` | 5.1-5.3 |
...
```typescript
// 强制约束: 必须通过 Zustand Actions 触发所有状态变更
// ❌ 禁止: 直接修改 store.approvalList = newList
// ✓ 正确: await store.fetchApprovalList(params)

// 强制约束: React 组件必须使用 useShallow 或 selector 订阅状态
const approvalList = useApprovalStore((state) => state.approvalList);
const isLoading = useApprovalStore((state) => state.loadingState.isLoading);
```
