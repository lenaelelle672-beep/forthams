...
| 交付物 | 具体内容 |
|--------|----------|
| `WO-API-201` | 提交工单接口 `POST /api/v1/work-orders` |
| `WO-API-202` | 获取审批列表 `GET /api/v1/work-orders/pending` |
| `WO-API-203` | 审批操作接口 `POST /api/v1/work-orders/{id}/approve` |
| `WO-API-204` | 驳回操作接口 `POST /api/v1/work-orders/{id}/reject` |
| `WO-SVC-001` | 状态机服务（状态转换规则引擎） |
| `WO-NOTIFY-001` | 通知服务（事件驱动通知分发） |
| `WO-FE-001` | 审批管理前端页面 |
...
async function testApprovalPageRendersPendingList(page: Page) {
...
async function testApproveActionUpdatesUI(page: Page) {
...
async function testRejectRequiresReason(page: Page) {
...
class WorkOrderStateMachine:
    ...
    def _get_next_state(self, state: str, action: str) -> str:
        ...
        return mapping.get(action, state)
...
```python
# repositories/work_order.py
class WorkOrderRepository:
    """工单仓储层 - 处理数据库操作"""
    
    async def approve_with_optimistic_lock(
        self,
        work_order_id: int,
        approver_id: int,
        version: int,
        ...
        if result.rowcount == 0:
            raise ConflictError(
                f"Work order {work_order_id} was modified by another user"
            )
        
        return await self.get_by_id(work_order_id)
```
...
class NotificationService:
    ...
    def dispatch(self, event: WorkOrderStatusChanged) -> None:
        """
        分发工单状态变更通知
        ...
        # 记录通知历史
        self._save_notification_record(event)
    
    def _send_email_notification(self, event: WorkOrderStatusChanged) -> None:
        """发送邮件通知"""
        work_order = self._get_work_order(event.work_order_id)
        recipient = work_order.requester_email
        ...
        template_name = templates.get(event.new_status)
        if template_name:
            self.email_service.send(
                ...
                context={
                    ...
                    "operator": self._get_operator_name(event.operator_id),
                    "reason": event.reason,
