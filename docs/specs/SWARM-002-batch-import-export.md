...
### 依赖现有架构
- 复用 `RetirementRepository` (L391) 数据持久化层
- 复用 `AssetRetirementRequest` (L153) 数据模型
- 集成现有 `Enum` 枚举体系
...
| 目标编号 | 实施目标 | 对应代码位置 |
|---------|---------|-------------|
| P1.1 | Excel 文件解析与校验 | `src/services/asset_import_service.py` (新建) |
| P1.2 | 字段映射引擎 | `src/services/field_mapping_engine.py` (新建) |
| P1.3 | 批量数据验证框架 | `src/validators/batch_validator.py` (新建) |
| P1.4 | 错误回显服务 | `src/services/import_error_service.py` (新建) |

### Phase 2: Excel 导出核心能力
| 目标编号 | 实施目标 | 对应代码位置 |
|---------|---------|-------------|
| P2.1 | 资产清单查询服务 | 扩展 `RetirementService` (L444) |
...
| 目标编号 | 实施目标 | 对应代码位置 |
|---------|---------|-------------|
| P3.1 | 导入 API 端点 | `src/main.py` 新增路由 L850+ |
| P3.2 | 导出 API 端点 | `src/main.py` 新增路由 L870+ |
...
src/
├── models/
│   ├── BatchImportRequest.py      # 批量导入请求模型
│   ├── BatchImportResult.py       # 导入结果模型
│   ├── BatchExportRequest.py      # 导出请求模型 ← 本次实现
│   └── ImportErrorDetail.py       # 错误详情模型
├── utils/
...
src/
├── services/
│   ├── FieldMappingEngine.py      # 字段映射引擎
│   ├── BatchValidator.py          # 批量验证器
│   ├── AssetImportService.py      # 导入服务
│   ├── AssetExportService.py      # 导出服务
│   └── ImportErrorService.py      # 错误回显服务
```
...
src/
├── repositories/
│   └── BatchImportRepository.py   # 批量导入记录持久化
```

### Layer 3: API 层（最后）
```
src/main.py 新增端点:
- POST /api/v1/assets/batch/import     # L860
- GET  /api/v1/assets/batch/export    # L880
- GET  /api/v1/assets/batch/import/{task_id}/errors  # L900
```
...
根据 `[排队执行任务限制]: 只聚焦文件：**src/models/BatchExportRequest.py**`，本次仅实现导出请求模型。

### 实现文件清单
| 优先级 | 文件 | 说明 |
|-------|------|------|
| P0 | `src/models/batch_export_request.py` | 导出请求模型 |

### 物理测试期待
```python
# tests/test_batch_export.py::TestBatchExportRequest
def test_batch_export_request_validates_required_fields():
    """物理测试：必填字段校验"""
    # Expect: asset_name, status 必填，缺失则 Pydantic ValidationError

def test_batch_export_request_parses_date_range():
    """物理测试：日期范围解析"""
    # Input: start_date="2024-01-01", end_date="2024-12-31"
    # Expect: start_date 和 end_date 正确解析为 datetime

def test_batch_export_request_validates_page_size():
    """物理测试：分页大小验证"""
    # Input: page_size=6000
    # Expect: 验证失败，最大值为 5000
```