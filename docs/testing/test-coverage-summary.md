...
| 后端测试 | `cd backend && mvn test -DfailIfNoTests=false` | 607 个测试通过，0 失败，0 错误 |
...
本轮补强多租户 SQL 防漏：`TenantLineInnerInterceptor` 对租户业务表缺少 TenantContext 的 SQL 改为显式拒绝，不再生成 `tenant_id = ''` 的静默空结果；后端全量 607 个测试通过，真实后端 E2E 9 个测试通过。
