...
```typescript
// Playwright E2E 测试
async function test_websocket_realtime_update(page: Page) {
  /* TC-020: WebSocket 实时推送验证 */
  await page.goto(`/workorders/${work_order_id}`);
...
│                         前端层 (Frontend)                        │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────────────────┐ │
│  │ApprovalActions│  │ApprovalHistory│  │    TransferModal      │ │
│  └──────┬──────┘  └──────┬──────┘  └───────────┬─────────────┘ │
└─────────┼────────────────┼────────────────────┼────────────────┘
          │                │                    │
          ▼                ▼                    ▼
...
│                     服务层 (Service Layer)                       │
│  ┌──────────────────┐  ┌──────────────────────────────────────┐│
│  │WorkOrderService  │  │        ApprovalService               ││
│  │ - create()       │  │  - approve(operator_id)              ││
│  │ - submit()       │  │  - reject(reason)                    ││
│  │ - get_status()   │  │  - transfer(target_user_id, reason)   ││
│  └────────┬─────────┘  └───────────────┬──────────────────────┘│
│           │                             │                       │
│           ▼                             ▼                       │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │              WorkOrderStateMachine                         │  │
│  │  PENDING → APPROVED | REJECTED | TRANSFERRED → CLOSED     │  │
│  └──────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
          │
          ▼
...
│  ┌────────────────────┐        ┌──────────────────────────────┐ │
│  │  WebSocket Manager │        │     Email Notification      │ │
│  │  (实时推送)         │        │     (异步任务队列)           │ │
│  └────────────────────┘        └──────────────────────────────┘│
└─────────────────────────────────────────────────────────────────┘
          │
...
│                     数据层 (Data Layer)                          │
│  ┌────────────────┐  ┌─────────────────┐  ┌───────────────────┐  │
│  │  WorkOrder     │  │ ApprovalHistory │  │   Notification    │  │
│  │  (工单主表)     │  │  (审批历史表)    │  │   Record         │  │
│  └────────────────┘  └─────────────────┘  └───────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
```
...
| id | Long | 工单唯一标识 |
| title | String | 工单标题 |
| status | Enum | 当前状态 (PENDING/APPROVED/REJECTED/TRANSFERRED/CLOSED) |
| creator_id | Long | 创建人ID |
| current_approver_id | Long | 当前审批人ID |
...
| action | Enum | 动作类型 (APPROVE/REJECT/TRANSFER) |
| reason | String | 理由/备注 |
...
1. **权限校验**: 只有 `current_approver_id` 对应的用户才能执行审批操作
2. **状态校验**: 已终态 (CLOSED) 的工单不可再进行状态变更
3. **审计日志**: 所有审批操作必须记录审计日志 (参见 AuditService)
4. **防重放**: API 请求需包含 timestamp 和 nonce 防止重放攻击
...
| 指标 | 目标值 | 说明 |
|------|--------|------|
| API 响应时间 (P99) | < 200ms | 单次审批操作 |
| WebSocket 推送延迟 | < 500ms | 从状态变更到推送完成 |
| Email 任务入队延迟 | < 1s | 审批触发到任务入队 |
| 历史查询响应时间 | < 100ms | 单工单历史记录查询 |