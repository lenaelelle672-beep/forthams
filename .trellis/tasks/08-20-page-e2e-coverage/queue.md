...
| Q3466 | in_progress | 桌面空态：交接管理状态列（状态/未闭环风险/发起推进取消）。禁 /m。 |
| Q3467 | in_progress | 桌面空态：技术支持状态列（状态/诊断包脱敏/处理状态）。禁 /m。 |
| Q3468 | in_progress | 桌面空态：导入导出状态列（状态/脱敏错误摘要/重试）。禁 /m。 |
| Q3469 | in_progress | 桌面空态：表单存储标签（字段摘要/附件摘要/可更新）。禁 /m。 |
| Q3470 | in_progress | 桌面空态：审计登录选项（LOGIN/不采集审计写入/点数 0）。禁 /m。 |
| Q3471 | in_progress | 桌面空态：文档发布状态（发布状态/编辑发布归档/删除留痕）。禁 /m。 |
| Q3472 | in_progress | 桌面空态：流程邮件发送边界（真实邮件未接入/测试发送/只读展示）。禁 /m。 |
| Q3473 | in_progress | 桌面空态：流程定义版本说明（版本状态/流程模板版本状态/businessType）。禁 /m。 |
| Q3474 | in_progress | 桌面空态：待办字段可见计数（0/0 可见/来源解释/preview 脱敏）。禁 /m。 |
| Q3475 | pending | 桌面空态：下一票继续 workbenchv3 未用角色。禁 /m。 |
