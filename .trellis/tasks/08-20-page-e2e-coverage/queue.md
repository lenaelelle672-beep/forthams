...
| Q3380 | in_progress | 桌面空态：租户管理 KPI（租户总数/正常租户/当前过滤结果）。禁 /m。 |
| Q3381 | in_progress | 桌面空态：租户管理操作（重新加载/搜索/只读边界）。禁 /m。 |
| Q3382 | in_progress | 桌面空态：租户管理状态筛（状态筛选/正常/已停用）。禁 /m。 |
| Q3383 | in_progress | 桌面空态：租户管理表头（租户标识/名称/套餐）。禁 /m。 |
| Q3384 | in_progress | 桌面空态：租户管理表头余项（用户上限/资产上限/联系人）。禁 /m。 |
| Q3385 | in_progress | 桌面空态：租户管理只读（状态列/显示 0 条/只读提示）。禁 /m。 |
| Q3386 | in_progress | 桌面空态：租户管理分页（上一页/下一页/第 n 页）。禁 /m。 |
| Q3387 | in_progress | 桌面空态：数据权限 KPI（角色总数/全部数据 ALL/已收紧范围）。禁 /m。 |
| Q3388 | in_progress | 桌面空态：数据权限操作（自定义 CUSTOM/重新加载/边界提示）。禁 /m。 |
| Q3389 | in_progress | 桌面空态：数据权限范围筛（数据范围筛选/全部数据/已收紧）。禁 /m。 |
| Q3390 | in_progress | 桌面空态：数据权限表头（角色编码/角色名称/数据范围）。禁 /m。 |
| Q3391 | in_progress | 桌面空态：数据权限表头余项（风险说明/自定义部门/显示 0 条）。禁 /m。 |
| Q3392 | in_progress | 桌面空态：数据权限分页（上一页/下一页/第 n 页）。禁 /m。 |
| Q3393 | in_progress | 桌面空态：角色权限 KPI（角色数量/权限库存/绑定数量）。禁 /m。 |
| Q3394 | in_progress | 桌面空态：角色权限操作（未绑定权限/重新加载/搜索）。禁 /m。 |
| Q3395 | in_progress | 桌面空态：角色权限状态筛（状态筛选/启用/停用）。禁 /m。 |
| Q3396 | in_progress | 桌面空态：角色权限只读（库存 heading/不支持分配/显示 0 角色）。禁 /m。 |
| Q3397 | in_progress | 桌面空态：角色权限表头（角色编码/角色名称/状态）。禁 /m。 |
| Q3398 | in_progress | 桌面空态：角色权限表头余项（已绑定数量/绑定权限/全部状态）。禁 /m。 |
| Q3399 | in_progress | 桌面空态：菜单权限 KPI（权限域数量/编码库存/已绑定权限）。禁 /m。 |
| Q3400 | in_progress | 桌面空态：菜单权限操作（未绑定权限/重新加载/搜索）。禁 /m。 |
| Q3401 | in_progress | 桌面空态：菜单权限筛选（关键词/状态筛选/启用）。禁 /m。 |
| Q3402 | in_progress | 桌面空态：菜单权限只读（停用/只读目录/sys_menu）。禁 /m。 |
| Q3403 | in_progress | 桌面空态：待办字段预览（预览可见字段/敏感字段脱敏/暂无预览）。禁 /m。 |
| Q3404 | in_progress | 桌面空态：待办字段角色（角色选择/APPROVER/AUDITOR）。禁 /m。 |
| Q3405 | in_progress | 桌面空态：待办字段操作（配置操作/保存字段配置/刷新预览）。禁 /m。 |
| Q3406 | in_progress | 桌面空态：表单存储操作（重新加载/真实调用/操作留痕）。禁 /m。 |
| Q3407 | in_progress | 桌面空态：表单存储摘要（未选择实例/字段摘要为空/附件引用为空）。禁 /m。 |
| Q3408 | in_progress | 桌面空态：表单存储按钮（创建示例实例/导出脱敏结果/尚未生成快照）。禁 /m。 |
| Q3409 | in_progress | 桌面空态：运行监控 KPI（待处理数量/当前页实例/总实例）。禁 /m。 |
| Q3410 | in_progress | 桌面空态：运行监控 SLA KPI（SLA 超时/预警/高风险轨迹）。禁 /m。 |
| Q3411 | in_progress | 桌面空态：运行监控筛选（重新加载/搜索/待处理）。禁 /m。 |
| Q3412 | in_progress | 桌面空态：运行监控表头（流程编号/SLA 异常轨迹/暂无异常轨迹）。禁 /m。 |
| Q3413 | in_progress | 桌面空态：流程邮件 KPI（配置总数/已启用/当前过滤结果）。禁 /m。 |
| Q3414 | in_progress | 桌面空态：流程邮件操作（重新加载/搜索/启用状态筛选）。禁 /m。 |
| Q3415 | in_progress | 桌面空态：流程邮件表头（业务类型/触发事件/显示 0 条）。禁 /m。 |
| Q3416 | in_progress | 桌面空态：导入导出 KPI 余项（失败任务/当前过滤结果/任务类型筛选）。禁 /m。 |
| Q3417 | in_progress | 桌面空态：导入导出筛选（任务状态筛选/导入/运行中）。禁 /m。 |
| Q3418 | in_progress | 桌面空态：导入导出表头（业务对象/成功失败/显示 0 条）。禁 /m。 |
| Q3419 | in_progress | 桌面空态：审批规则操作（操作留痕/模拟结果/冲突检测）。禁 /m。 |
| Q3420 | in_progress | 桌面空态：审批规则按钮（创建示例规则/模拟命中/尚未运行模拟）。禁 /m。 |
| Q3421 | in_progress | 桌面空态：审批规则其余（未选择规则/暂无命中/暂无冲突）。禁 /m。 |
| Q3422 | pending | 桌面空态：下一票继续 workbenchv3 未用角色。禁 /m。 |
