...
| Q3145 | in_progress | 桌面空态：自定义字段 heading（自定义字段/字段定义 catalog/详情预览）。禁 /m。 |
| Q3146 | pending | 桌面空态：自定义字段检索（搜索占位/关键词/查询只读目录）。禁 /m。 |
| Q3147 | pending | 桌面空态：自定义字段空表预览（暂无定义/请选择字段/无持久化预览）。禁 /m。 |
| Q3148 | pending | 桌面空态：SLA heading（SLA 配置/策略列表/策略详情）。禁 /m。 |
| Q3149 | pending | 桌面空态：SLA 空表（暂无策略/暂无超时/重新加载）。禁 /m。 |
| Q3150 | pending | 桌面空态：SLA 分区 heading（操作留痕/模拟结果/超时记录）。禁 /m。 |
| Q3151 | pending | 桌面空态：SLA 操作按钮（保存草稿/运行模拟/导出脱敏）。禁 /m。 |
| Q3152 | pending | 桌面空态：SLA 未选择（尚未模拟/未选择策略/暂无审计摘要）。禁 /m。 |
| Q3153 | pending | 桌面空态：字段集 heading（字段集/catalog/暂无字段集）。禁 /m。 |
| Q3154 | pending | 桌面空态：字段集检索（搜索占位/关键词/查询只读字段集）。禁 /m。 |
| Q3155 | pending | 桌面空态：字段集诊断（分类 ID/查询分类/请选择字段集）。禁 /m。 |
| Q3156 | pending | 桌面空态：字段集预览（预览字段 ID/无持久化预览/无只读匹配）。禁 /m。 |
