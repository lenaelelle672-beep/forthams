...
| Q3135 | in_progress | 桌面空态：检验模板检查项占位（外观检查/性能测试/精度校验）。禁 /m。 |
| Q3136 | pending | 桌面空态：采购其余状态筛选（已收货/已取消/草稿）。禁 /m。 |
| Q3137 | pending | 桌面空态：编号规则 heading（编号规则/catalog/详情预览）。禁 /m。 |
| Q3138 | pending | 桌面空态：编号规则 textbox（模板/样例时间/样例序号）。禁 /m。 |
| Q3139 | pending | 桌面空态：编号规则操作与空表（读取详情/无持久化预览/暂无编号规则）。禁 /m。 |
| Q3140 | pending | 桌面空态：编号规则编码与计数（规则编码/资产编号规则/显示 0 条）。禁 /m。 |
| Q3141 | pending | 桌面空态：编号规则 landmark（只读列表/详情/变量说明）。禁 /m。 |
| Q3142 | pending | 桌面空态：编号规则边界文案（真实调用/不会持久化/并发唯一）。禁 /m。 |
| Q3143 | pending | 桌面空态：编号规则覆盖边界（创建链路/基础资料组/44/44）。禁 /m。 |
| Q3144 | pending | 桌面空态：编号规则只读徽标（tenantScoped/元数据未加载/不分配序列号）。禁 /m。 |
