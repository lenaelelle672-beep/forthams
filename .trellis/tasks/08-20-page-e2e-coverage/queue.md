...
| Q3178 | in_progress | 桌面空态：基础参数其余默认键（默认币种/维保预警天数/备份频率）。禁 /m。 |
| Q3179 | pending | 桌面空态：基础参数审计字段（审计原因/操作人 ID/默认键 heading）。禁 /m。 |
| Q3180 | pending | 桌面空态：用户管理空表（用户管理/用户列表/暂无用户数据。）。禁 /m。 |
| Q3181 | pending | 桌面空态：用户管理检索（搜索占位/用户关键词/搜索）。禁 /m。 |
| Q3182 | pending | 桌面空态：邮件日志空表（邮件日志/日志 catalog/暂无可展示）。禁 /m。 |
| Q3183 | pending | 桌面空态：邮件日志检索（模板编码/业务类型/业务 ID）。禁 /m。 |
| Q3184 | pending | 桌面空态：邮件日志详情（脱敏详情/暂无业务关联/请选择日志）。禁 /m。 |
| Q3185 | pending | 桌面空态：通知模板空表（通知模板/暂无通知模板/关键词）。禁 /m。 |
| Q3186 | pending | 桌面空态：通知模板预览（templateCode/按编码加载/无持久化预览）。禁 /m。 |
