...
| Q3306 | in_progress | 桌面空态：文件存储 KPI（附件总数/总容量/业务类型）。禁 /m。 |
| Q3307 | pending | 桌面空态：文件存储筛选余项（业务类型筛选/文件类型筛选/文件类型）。禁 /m。 |
| Q3308 | pending | 桌面空态：文件存储表头（文件名/业务 ID/创建时间）。禁 /m。 |
| Q3309 | pending | 桌面空态：文件存储分页（上一页/下一页/第 1 页）。禁 /m。 |
| Q3310 | pending | 桌面空态：文件存储只读（只读元数据目录/不支持上传/仅展示附件元数据）。禁 /m。 |
| Q3311 | pending | 桌面空态：缓存管理余项（白名单/CacheManager/空缓存不会被报告）。禁 /m。 |
| Q3312 | pending | 桌面空态：同步规则余项（只读队列摘要/dryRun/队列消费已禁用）。禁 /m。 |
| Q3313 | pending | 桌面空态：安全策略提示余项（会话最长时长/MFA/操作人 ID）。禁 /m。 |
| Q3314 | pending | 桌面空态：安全策略审计余项（保存配置态/审计证据摘要/不改变会话）。禁 /m。 |
| Q3315 | pending | 桌面空态：流程控制台模板表头（名称/状态/版本）。禁 /m。 |
| Q3316 | pending | 桌面空态：流程控制台运行表头（业务 ID/当前步骤/更新时间）。禁 /m。 |
| Q3317 | pending | 桌面空态：流程控制台健康余项（流程类型覆盖/数据来源/脱敏导出提示）。禁 /m。 |
| Q3318 | pending | 桌面空态：流程设计器节点字段（节点说明/节点编码/审批角色）。禁 /m。 |
| Q3319 | pending | 桌面空态：流程设计器节点表单（区段名称/历史摘要字段/子表单 HTML）。禁 /m。 |
| Q3320 | pending | 桌面空态：流程设计器节点辅助（适合表单提交/流程入口/reason,amount）。禁 /m。 |
| Q3321 | pending | 桌面空态：表单配置其余勾选（遮罩/桌面H5双端/字段表）。禁 /m。 |
| Q3322 | pending | 桌面空态：表单配置布局（双列布局/单列移动端/敏感字段默认遮罩）。禁 /m。 |
| Q3323 | pending | 桌面空态：表单配置状态（章节字段/UNCONFIGURED/必填）。禁 /m。 |
| Q3324 | pending | 桌面空态：资产分类表头（编码/父级 ID/排序）。禁 /m。 |
| Q3325 | pending | 桌面空态：流程控制台 SLA 列（节点/SLA 节点耗时/SLA 异常轨迹）。禁 /m。 |
| Q3326 | pending | 桌面空态：流程设计器审批模式（审批模式/依次审批/用于发起页渲染）。禁 /m。 |
| Q3327 | pending | 桌面空态：文件存储列余项（类型/大小/上传人）。禁 /m。 |
| Q3328 | pending | 桌面空态：安全策略配置提示（登录失败限制/审计记录开关/configuration-state）。禁 /m。 |
| Q3329 | pending | 桌面空态：安全策略复核（当前复核对象/operatorId/masked before）。禁 /m。 |
| Q3330 | pending | 桌面空态：同步规则队列（待处理/运行中/批处理队列消费入口不开放）。禁 /m。 |
| Q3331 | pending | 桌面空态：流程控制台页码（显示 0/0 条/第 1/0 页/脱敏展示 0 条）。禁 /m。 |
| Q3332 | pending | 桌面空态：流程设计器节点描述（会签规则/非阻塞通知/字段值分流）。禁 /m。 |
| Q3333 | pending | 桌面空态：表单配置工作台说明（字段列表/不影响已发布版本/form-definitions）。禁 /m。 |
| Q3334 | pending | 桌面空态：文件存储页脚（0/0 条元数据/0 B/业务类型列）。禁 /m。 |
| Q3335 | pending | 桌面空态：安全策略当前配置（当前配置/未返回或已脱敏/runtimeEffect）。禁 /m。 |
