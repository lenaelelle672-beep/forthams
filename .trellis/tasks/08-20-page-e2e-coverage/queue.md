...
| Q3359 | in_progress | 桌面空态：交接管理 KPI（交接任务总数/待处理进行中/当前过滤结果）。禁 /m。 |
| Q3360 | pending | 桌面空态：交接管理操作（重新加载/搜索/交接状态筛选）。禁 /m。 |
| Q3361 | pending | 桌面空态：交接管理状态筛（待交接/交接中/已取消）。禁 /m。 |
| Q3362 | pending | 桌面空态：交接管理表头（标题/交接人/接收人）。禁 /m。 |
| Q3363 | pending | 桌面空态：交接管理表头余项（资产/工单/审批）。禁 /m。 |
| Q3364 | pending | 桌面空态：交接管理只读（只读边界/只读提示/只读展示摘要）。禁 /m。 |
| Q3365 | pending | 桌面空态：文档中心 KPI（文档总数/已发布/重新加载）。禁 /m。 |
| Q3366 | pending | 桌面空态：文档中心筛选（文档状态/草稿/已归档）。禁 /m。 |
| Q3367 | pending | 桌面空态：文档中心表头（分类/作者/附件）。禁 /m。 |
| Q3368 | pending | 桌面空态：文档中心只读（只读边界/只读展示文档标题/搜索）。禁 /m。 |
| Q3369 | pending | 桌面空态：技术支持 KPI（工单总数/待处理处理中/安全边界）。禁 /m。 |
| Q3370 | pending | 桌面空态：技术支持优先级（全部优先级/紧急/工单优先级筛选）。禁 /m。 |
| Q3371 | pending | 桌面空态：技术支持表头（优先级/提单人/诊断包）。禁 /m。 |
| Q3372 | pending | 桌面空态：技术支持只读（处理人/禁止导出敏感配置/只读展示工单）。禁 /m。 |
