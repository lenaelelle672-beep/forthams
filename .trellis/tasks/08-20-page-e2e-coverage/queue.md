...
| Q3422 | in_progress | 桌面空态：待办字段按钮余项（保存稳定排序/保存角色覆盖/恢复默认配置）。禁 /m。 |
| Q3423 | in_progress | 桌面空态：待办字段说明（强制租户隔离/当前角色/USER）。禁 /m。 |
| Q3424 | in_progress | 桌面空态：表单存储标题余项（maskedValue/附件脱敏引用/归档留痕）。禁 /m。 |
| Q3425 | in_progress | 桌面空态：表单存储按钮余项（更新字段摘要/登记附件引用/导出脱敏快照）。禁 /m。 |
| Q3426 | in_progress | 桌面空态：表单存储留痕（尚未归档/尚未删除/服务层强制租户隔离）。禁 /m。 |
| Q3427 | in_progress | 桌面空态：运行监控表头余项（流程类型/业务 ID/当前步骤）。禁 /m。 |
| Q3428 | in_progress | 桌面空态：运行监控列与状态（申请人/发起时间/已完成）。禁 /m。 |
| Q3429 | in_progress | 桌面空态：运行监控 SLA 余项（已退回/只读边界/超时记录 heading）。禁 /m。 |
| Q3430 | in_progress | 桌面空态：流程邮件表头余项（节点/模板/收件人）。禁 /m。 |
| Q3431 | in_progress | 桌面空态：流程邮件分页（上一页/下一页/第 n 页）。禁 /m。 |
| Q3432 | in_progress | 桌面空态：流程邮件只读余项（只读边界/已停用/业务类型）。禁 /m。 |
| Q3433 | in_progress | 桌面空态：导入导出表头余项（ID/类型/格式）。禁 /m。 |
| Q3434 | in_progress | 桌面空态：导入导出列余项（操作人/错误摘要/时间）。禁 /m。 |
| Q3435 | in_progress | 桌面空态：导入导出分页（上一页/下一页/第 n 页）。禁 /m。 |
| Q3436 | in_progress | 桌面空态：导入导出筛选项余项（只读边界/成功/已取消）。禁 /m。 |
| Q3437 | in_progress | 桌面空态：审批规则按钮余项（更新优先级/启用规则/停用规则）。禁 /m。 |
| Q3438 | in_progress | 桌面空态：审批规则摘要（冲突检测/暂无条件摘要/尚未启用）。禁 /m。 |
| Q3439 | in_progress | 桌面空态：审批规则边界（白名单 parser/候选处理人/0 条）。禁 /m。 |
| Q3440 | in_progress | 桌面空态：审计日志 KPI（命中日志/统计总数/操作类型）。禁 /m。 |
| Q3441 | in_progress | 桌面空态：审计日志筛选（操作类型筛选/资源类型筛选/全部资源类型）。禁 /m。 |
| Q3442 | in_progress | 桌面空态：审计日志只读（只读审计日志/非闭环/不创建 Blob）。禁 /m。 |
| Q3443 | in_progress | 桌面空态：审计日志列表余项（列表 region/脱敏摘要/显示 0 条）。禁 /m。 |
| Q3444 | in_progress | 桌面空态：文档中心余项（当前过滤结果/只读提示/附件安全）。禁 /m。 |
| Q3445 | in_progress | 桌面空态：文档中心表头余项（标题/状态/显示 0 条）。禁 /m。 |
| Q3446 | in_progress | 桌面空态：技术支持余项（高/低/当前过滤结果）。禁 /m。 |
| Q3447 | in_progress | 桌面空态：数据权限只读余项（只读 catalog/可调整范围/自定义）。禁 /m。 |
| Q3448 | in_progress | 桌面空态：租户管理说明余项（/tenants/写操作/V3 catalog）。禁 /m。 |
| Q3449 | in_progress | 桌面空态：运行监控导出提示（脱敏摘要/页码/状态列）。禁 /m。 |
| Q3450 | in_progress | 桌面空态：审计日志趋势余项（粒度 daily/趋势 region/CREATE）。禁 /m。 |
| Q3451 | pending | 桌面空态：下一票继续 workbenchv3 未用角色。禁 /m。 |
