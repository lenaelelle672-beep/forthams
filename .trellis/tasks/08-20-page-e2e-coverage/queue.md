...
| Q3271 | in_progress | 桌面空态：流程控制台余项（重新加载/运行健康摘要/暂无 SLA 超时记录）。禁 /m。 |
| Q3272 | pending | 桌面空态：流程控制台 KPI（待处理数量/流程模板总数/已配置模板）。禁 /m。 |
| Q3273 | pending | 桌面空态：流程控制台筛选（模板状态/实例状态/搜索）。禁 /m。 |
| Q3274 | pending | 桌面空态：流程控制台健康（暂无运行实例/暂无异常耗时/暂无异常轨迹）。禁 /m。 |
| Q3275 | pending | 桌面空态：流程控制台 SLA（导出仅返回脱敏摘要/只读联动/运行实例总数）。禁 /m。 |
| Q3276 | pending | 桌面空态：资产分类余项（分类树/重新加载/搜索）。禁 /m。 |
| Q3277 | pending | 桌面空态：流程设计器校验（校验与审计证据/尚未执行图结构校验/版本历史）。禁 /m。 |
| Q3278 | pending | 桌面空态：流程设计器操作（载入设计/保存草稿/图结构校验）。禁 /m。 |
| Q3279 | pending | 桌面空态：流程设计器发布（发布流程/恢复最近版本/草稿名称）。禁 /m。 |
| Q3280 | pending | 桌面空态：流程设计器草稿（草稿说明/拖拽环节节点/连线分支）。禁 /m。 |
| Q3281 | pending | 桌面空态：表单配置定义（当前定义/版本历史/暂无发布停用回滚版本）。禁 /m。 |
| Q3282 | pending | 桌面空态：表单配置校验（schema 校验与遮罩预览/尚未执行/引用分析）。禁 /m。 |
| Q3283 | pending | 桌面空态：表单配置操作（载入表单/新建表单/保存草稿）。禁 /m。 |
| Q3284 | pending | 桌面空态：表单配置发布（schema 校验/发布表单/停用表单）。禁 /m。 |
| Q3285 | pending | 桌面空态：表单配置预览（字段表/桌面预览/钉钉 H5 预览）。禁 /m。 |
| Q3286 | pending | 桌面空态：表单配置属性（字段属性/草稿名称/草稿说明）。禁 /m。 |
| Q3287 | pending | 桌面空态：表单配置字段键（字段名称/字段 Key/字段类型）。禁 /m。 |
| Q3288 | pending | 桌面空态：表单配置默认字段（资产编号/联系方式/系统生成或手工录入）。禁 /m。 |
| Q3289 | pending | 桌面空态：表单配置字段操作（新增分组/新增字段/从草稿移除字段）。禁 /m。 |
| Q3290 | pending | 桌面空态：表单配置分组输入（分组名称/选项占位/当前表单）。禁 /m。 |
| Q3291 | pending | 桌面空态：流程控制台表头（业务类型/实例编号/SLA 风险）。禁 /m。 |
| Q3292 | pending | 桌面空态：流程控制台说明（只读风险提示/健康提示/SLA 策略）。禁 /m。 |
| Q3293 | pending | 桌面空态：流程设计器属性（右侧属性配置/状态版本/节点连线）。禁 /m。 |
| Q3294 | pending | 桌面空态：表单配置其余（恢复最近版本/基础信息/占位提示）。禁 /m。 |
| Q3295 | pending | 桌面空态：表单配置遮罩输入（默认值遮罩预览/帮助说明/选项配置）。禁 /m。 |
| Q3296 | pending | 桌面空态：表单配置勾选（分组说明/必填/敏感）。禁 /m。 |
| Q3297 | pending | 桌面空态：表单配置引用（引用数量/references/fail-closed）。禁 /m。 |
| Q3298 | pending | 桌面空态：资产分类说明（只读展示/共 0 条/名称列）。禁 /m。 |
| Q3299 | pending | 桌面空态：流程设计器说明（真实调用 workflows/发布与版本恢复/最后更新时间）。禁 /m。 |
| Q3300 | pending | 桌面空态：流程控制台检索余项（全部实例/流程类型/模板配置率）。禁 /m。 |
| Q3301 | pending | 桌面空态：流程设计器节点面板（节点面板/开始节点/审批节点）。禁 /m。 |
| Q3302 | pending | 桌面空态：流程设计器节点类型（办理节点/抄送节点/条件分支）。禁 /m。 |
| Q3303 | pending | 桌面空态：流程设计器节点余项（结束节点/拖拽到画布/节点属性）。禁 /m。 |
| Q3304 | pending | 桌面空态：流程设计器节点配置（节点名称/审批人类型/按角色）。禁 /m。 |
| Q3305 | pending | 桌面空态：流程设计器节点操作（指定用户/删除当前节点/环节子表单）。禁 /m。 |
