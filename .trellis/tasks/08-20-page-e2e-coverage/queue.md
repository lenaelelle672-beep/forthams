...
| Q3585 | pending | 桌面空态（流程平台侧栏 实例字段/策略计算/审批实例）。禁 /m。 |
| Q3586 | pending | 桌面空态（流程平台侧栏 流程模板/节点摘要/发布停用）。禁 /m。 |
| Q3587 | pending | 桌面空态（组织权限侧栏 根部门/岗位 catalog/CUSTOM）。禁 /m。 |
| Q3588 | pending | 桌面空态（组织权限侧栏 数据范围/交接/租户 catalog）。禁 /m。 |
| Q3589 | pending | 桌面空态（基础资料侧栏 分类树/供应商/根位置）。禁 /m。 |
| Q3590 | pending | 桌面空态（基础资料侧栏 无序号预览/字段定义/分类诊断）。禁 /m。 |
| Q3591 | pending | 桌面空态（消息通知侧栏 零业务调用/流程邮件/渠道 catalog）。禁 /m。 |
| Q3592 | pending | 桌面空态（消息通知侧栏 偏好/开关/metadata-only）。禁 /m。 |
| Q3593 | pending | 桌面空态（系统参数侧栏 附件元数据/任务历史/技术支持）。禁 /m。 |
| Q3594 | pending | 桌面空态（系统参数侧栏 GET-only 审计/文档目录/错误摘要）。禁 /m。 |
| Q3595 | pending | 桌面空态（集成配置侧栏 认证掩码/config-only/队列摘要）。禁 /m。 |
| Q3596 | pending | 桌面空态（系统参数侧栏 命名空间/SECURITY/SYSTEM 参数）。禁 /m。 |
| Q3597 | pending | 桌面空态：下一票继续 workbenchv3 未用角色。禁 /m。 |
