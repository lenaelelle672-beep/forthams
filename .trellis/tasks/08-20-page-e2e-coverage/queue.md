...
| Q3169 | in_progress | 桌面空态：位置管理空表（位置管理/列表/暂无位置数据。）。禁 /m。 |
| Q3170 | pending | 桌面空态：位置管理根位置（暂无根位置/根位置摘要/搜索占位）。禁 /m。 |
| Q3171 | pending | 桌面空态：位置管理检索（位置关键词/搜索/重新加载）。禁 /m。 |
| Q3172 | pending | 桌面空态：基础参数空表（基础参数/SYSTEM 目录/暂无 SYSTEM）。禁 /m。 |
| Q3173 | pending | 桌面空态：基础参数表单（保存基础参数/参数键/参数名称）。禁 /m。 |
| Q3174 | pending | 桌面空态：基础参数操作（参数值/影响预演/刷新缓存）。禁 /m。 |
| Q3175 | pending | 桌面空态：基础参数默认键（公司名称/系统名称/默认时区）。禁 /m。 |
| Q3176 | pending | 桌面空态：SLA 计数文案（策略总数/脱敏摘要/后端种子）。禁 /m。 |
| Q3177 | pending | 桌面空态：自定义字段边界文案（预览样例/真实调用/不代表字段集）。禁 /m。 |
