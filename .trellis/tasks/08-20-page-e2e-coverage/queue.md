...
| Q3597 | pending | 桌面空态（消息侧栏 预览旗标/meta 边界）。禁 /m。 |
| Q3598 | pending | 桌面空态（消息侧栏 免打扰/safe preview/模板 catalog）。禁 /m。 |
| Q3599 | pending | 桌面空态（组织侧栏 岗位 preview/用户只读列表）。禁 /m。 |
| Q3600 | pending | 桌面空态（资料侧栏 字段定义/字段集/编号规则）。禁 /m。 |
| Q3601 | pending | 桌面空态（页头 技术支持 catalog/权限专项/专属 API）。禁 /m。 |
| Q3602 | pending | 桌面空态（页脚 设计器/表单配置/待办字段）。禁 /m。 |
| Q3603 | pending | 桌面空态（页脚 字段映射/同步规则/Webhook）。禁 /m。 |
| Q3604 | pending | 桌面空态（页脚 角色/菜单/数据权限）。禁 /m。 |
| Q3605 | pending | 桌面空态（页脚 控制台/流程定义/运行监控）。禁 /m。 |
| Q3606 | pending | 桌面空态（页脚 通知模板/渠道/偏好）。禁 /m。 |
| Q3607 | pending | 桌面空态（页脚 自定义字段/字段集/编号规则）。禁 /m。 |
| Q3608 | pending | 桌面空态（页脚 安全策略/文件存储/缓存）。禁 /m。 |
| Q3609 | pending | 桌面空态：下一票继续 workbenchv3 未用角色。禁 /m。 |
