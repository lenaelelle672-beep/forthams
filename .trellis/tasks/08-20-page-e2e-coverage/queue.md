...
| Q3243 | in_progress | 桌面空态：菜单权限空表（菜单权限/暂无菜单权限编码目录/权限域占位）。禁 /m。 |
| Q3244 | pending | 桌面空态：待办字段空表（待办字段配置/字段列表/暂无待办字段）。禁 /m。 |
| Q3245 | pending | 桌面空态：表单存储空表（表单存储/实例列表/暂无表单实例）。禁 /m。 |
| Q3246 | pending | 桌面空态：运行监控空表（运行监控/审批实例列表/暂无审批实例）。禁 /m。 |
| Q3247 | pending | 桌面空态：运行监控检索（流程类型占位/流程类型/全部状态）。禁 /m。 |
| Q3248 | pending | 桌面空态：审计日志分区（趋势/操作类型分布/操作人排行）。禁 /m。 |
| Q3249 | pending | 桌面空态：审计日志边界（真实调用 /audit-logs/GET-only/查询）。禁 /m。 |
| Q3250 | pending | 桌面空态：外部系统余项（系统编码/仅提交不回显/审计原因）。禁 /m。 |
| Q3251 | pending | 桌面空态：审批规则余项（真实调用 /approval-rules/重新加载/规则详情）。禁 /m。 |
| Q3252 | pending | 桌面空态：角色权限检索（角色搜索占位/角色权限关键词/暂无权限库存）。禁 /m。 |
| Q3253 | pending | 桌面空态：交接管理检索（交接人占位/交接任务关键词）。禁 /m。 |
| Q3254 | pending | 桌面空态：文档中心检索（标题摘要占位/文档关键词）。禁 /m。 |
| Q3255 | pending | 桌面空态：技术支持检索（标题摘要占位/工单关键词）。禁 /m。 |
| Q3256 | pending | 桌面空态：租户管理检索（租户标识占位/租户关键词）。禁 /m。 |
| Q3257 | pending | 桌面空态：数据权限检索（角色名称占位/角色关键词/全部范围）。禁 /m。 |
| Q3258 | pending | 桌面空态：待办字段余项（重新加载/真实调用 /todo-fields/角色覆盖）。禁 /m。 |
| Q3259 | pending | 桌面空态：表单存储余项（实例详情/暂无字段摘要/暂无附件引用）。禁 /m。 |
| Q3260 | pending | 桌面空态：运行监控 SLA（节点耗时/暂无节点耗时异常/暂无超时记录）。禁 /m。 |
