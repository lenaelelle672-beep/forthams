...
- 空态/403/失败态：`src/e2e/app-pages-empty-error.spec.ts`。失败态只测页面会渲染「加载失败」的路径；Dashboard KPI 失败不显示该文案，不要硬断言。桌面 `/inspection-templates`、`/inventory/cycle-count`、`/inventory/abc-classification` 空表文案分别为「暂无检验模板」「暂无循环盘点规则」「暂无资产分类数据」（antd `locale.emptyText`）。
