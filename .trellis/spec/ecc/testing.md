...
- workbenchv3 桌面空态 Q3261+：流程控制台用 `mockCommandCenterEmpty`（`/workflows` 必须数组）；流程设计器用 `mockFlowDesignerEmpty`（`/workflows` 与 `*/versions` 必须数组）；表单配置用 `mockFormDefinitionsEmpty`（`/form-definitions` 必须数组）；资产分类可用默认 `mockApi`（`/categories/tree` 已是 `[]`）。部门组织用 `mockDeptsListEmpty`（`/depts/list` 必须数组，否则 flatten 崩进 V3 error boundary）；位置用 `mockLocationsListEmpty`；供应商用 `mockVendorsListEmpty`。Q3271–Q3474 覆盖控制台至交接/技术支持边角。Q3475–Q3479 覆盖用户管理未用角色。Q3480–Q3502 覆盖部门组织/位置/供应商/岗位余项与基础参数未用角色。Q3503–Q3523 覆盖邮件模板/邮件日志/通知模板未用角色。禁 `/m`。下一票 Q3524。
